...
export class PlayerController {
  ...
  attach(
    ...
    this.lane = 0;
    this.column = Phaser.Math.Clamp(options.startColumn, 0, metrics.columns - 1);
    this.moving = false;
    this.enabled = true;
  ...
  requestMove(direction: MoveDirection): void {
    ...
    const toX = this.metrics.columnX(next.column);
    ...
    this.scene.tweens.addCounter({
      ...
      onUpdate: (tween) => {
        ...
        const y = Phaser.Math.Linear(fromY, toY, progress) - Math.sin(progress * Math.PI) * 14;
      ...
      onComplete: () => {
        ...
        this.options.onPosition(toX, toY);
        this.moving = false;
  ...
  private resolveDestination(direction: MoveDirection): { lane: number; column: number } | null {
    ...
    return {
      lane: Phaser.Math.Clamp(this.lane + laneDelta, 0, this.metrics.laneCount - 1),
      column: Phaser.Math.Clamp(this.column + columnDelta, 0, this.metrics.columns - 1),
    };
  }
